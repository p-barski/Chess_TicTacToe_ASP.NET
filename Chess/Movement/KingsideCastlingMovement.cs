...
{
	///<summary>
	///Responsible for calculating a possibility of kingside castling for a king.
	///</summary>
	public class KingsideCastlingMovement : IMovement
	{
		private const int boardSize = 8;
		private const int kingsideRookXPosition = 7;
		private readonly IChessBoard chessBoard;
		private readonly IMovement movement;
		public KingsideCastlingMovement(IChessBoard chessBoard, IMovement movement)
		{
		...
		private IReadOnlyChessPiece GetQueensideRook(IReadOnlyChessPiece king)
		{
			return chessBoard.Pieces
				.FirstOrDefault(p =>
					...
					p.HasMoved == false &&
					p.Position == new Position(kingsideRookXPosition, king.Position.Y));
		}
